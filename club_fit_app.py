...
with st.sidebar:
    ...
    if _player_csv_names:
        _player_choice = st.selectbox("Player CSV", _player_csv_names, index=0, key="cf_player_sel")
    ...
    if _team_csv_names:
        _team_choice = st.selectbox("Team Stats CSV", _team_csv_names, index=0, key="cf_team_sel")
...
# ── RUN ───────────────────────────────────────────────────────────────
run = st.button("🔍  Find Club Fits", type="primary", width="stretch")
...
if st.session_state.get('cf_rank_img'):
    st.image(st.session_state['cf_rank_img'], width="stretch")
